...
// Collector manages custom controller metrics for observability.
...
// newCollector creates a new Collector with all metrics initialized.
...
func (m *Collector) RecordReconcileDuration(controller string, duration time.Duration, result string) {
...
func (m *Collector) RecordReconcileError(controller, errorType string) {
...
// result: "success" or "error"
func (m *Collector) RecordProvisioningStep(controller, step, result string) {
...
func (m *Collector) SetQueueDepth(controller string, depth float64) {
...
func (m *Collector) NewReconcileTimer(controller string) *ReconcileTimer {
...
func ClassifyError(err error) string {
	...
	case containsAny(errStr, "not found", "does not exist"):
		return "not_found"
	case containsAny(errStr, "canceled", "context canceled"):
		return "canceled"
	default:
		return "unknown"
	}
